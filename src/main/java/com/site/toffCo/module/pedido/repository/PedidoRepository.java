...
public interface PedidoRepository extends JpaRepository<Pedido, UUID> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT p FROM Pedido p WHERE p.id = :pedidoId AND p.user.id = :userId")
    Optional<Pedido> findByIdAndUserIdForUpdate(@Param("pedidoId") UUID pedidoId, @Param("userId") UUID userId);
